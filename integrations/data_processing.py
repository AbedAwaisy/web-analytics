...
# helper function [no need to do any thing]
def connect():
...
#####################################################################################################################
# Integration Functions [need to do, here the interaction with the user begins]
def HerbName_Extractor(df):
    ...
    if ('זן' in columns.keys()) and df[columns['זן']].isnull().sum() != len(df):
        ...
        if len(herbs_df) == 1 and (0 < none_count / len(df) <= 0.3):
            ...
            authority = input(
                'There is values of column זן that is missing , please correct the values or give me authority to delete those rows: ')
            if authority == 'y':
    ...
    elif len(rokev) != 0 or len(knh) != 0:
        if len(rokev) != 0:
            df['HerbName'] = str(df[columns[rokev[0]]]).strip()
        else:
            df['HerbName'] = str(df[columns[knh[0]]]).strip()
...
def HerbName_Integrator(df, column_name='HerbName', table_name='ParcelMetaData'):
    columns = columns_mapper(df)
...
def ParcelSize_Integrator(df):
    ...
    if is_empty_dbs():
        ...
        if (df[column_name] == 0).all() or none_count == len(df):
            ...
            authority = input(
                'The values of column גודל חלקה במר is not valid, some of the Parcels with a size of 0 or empty. please correct the values or give me authority to delete those rows: ')
            if authority == True:
        ...
        connection = connect()
        cursor = connection.cursor()
        query = "SELECT ParcelID, ParcelSize from ParcelMetaData"
...
def extract_meta(df):
    ...
    meta = pd.DataFrame(
        {'SampleID': SampleID, 'ParcelID': ParcelID, 'SortingDate': SortingDate, 'HarvestDate': HarvestDate,
         'HerbName': HerbName, 'ParcelSize': ParcelSize, 'SortingType': SortingType}, index=df.index)

    return meta, df_ParcelSize


# when we call Intergrate function first we call extract meta and there is the begin of the inertaction with the user
# the order as follows:
# 1. herb name need to focus on the messages with the raise function and return to the user the message
# do the "HerbName_Extractor" function
# 2. parcel size need to focus on the messages with the raise function and return to the user the message
# do ParcelSize_Integrator function

################################################################################################################
# all the insert functions only needs to print a message to the user nothing more. (no user interaction)
...
def Insert_Cemical_With_Experiment(cemical2):
    ...
    try:
        ...
        for index, row in cemical2.iterrows():
            cursor.execute(sql, (
            row['SampleID'], row['ExperimentType'], row['ExperimentParameter'], row['TSS'], row['TA'], row['Glucose']))
        # Commit the transaction
        connection.commit()
        print("Data inserted successfully into MySQL table:", table_name)
...
def Insert_Quality_With_Experiment(Quality_df):
    ...
    # Insert data into MySQL table
    try:
        Quality_df.columns = ['SampleID', 'ExperimentType', 'ExperimentParameter', 'Weight', 'FruietNumber',
...
def Insert_Quality_Without_Experiment(Quality_df):
    ...
    # Insert data into MySQL table
    try:
        Quality_df.columns = ['SampleID', 'Weight', 'FruietNumber', 'VineFreshness', 'Fallen', 'Cracked', 'Firm',
...
def Insert_Yield_With_Experiment(Yield_df):
    ...
    try:
        ...
        Yield_df.columns = ['SampleID', 'ExperimentType', 'ExperimentParameter', 'ClusterHarvesd', 'SingleHarvesd',
        ...
        for index, row in Yield_df.iterrows():
            ...
            row['BlackPitDefect'], row['BlackPitDefectNumber'], row['Others'], row['PlantProtection'], row['Virus']))
        # Commit the transaction
        connection.commit()
        print("Data inserted successfully into MySQL table:", table_name)
...
def Insert_Yield_Without_Experiment(Yield_df):
    ...
    try:
        Yield_df.replace({np.nan: None}, inplace=True)
        Yield_df.columns = ['SampleID', 'ClusterHarvesd', 'SingleHarvesd', 'SingleHarvesdNumber', 'GreenFruits',
        ...
        for index, row in Yield_df.iterrows():
            ...
            row['GreenFruits'], row['CrackedFruits'], row['BlackPitDefect'], row['BlackPitDefectNumber'], row['Others'],
            row['PlantProtection'], row['Virus']))
        # Commit the transaction
        connection.commit()
        print("Data inserted successfully into MySQL table:", table_name)
...
#########################################################################################################################
# all the extraction functions [no need to do]
...
def extract_cemical(df, experiment):
    ...
    if experiment:
        ...
        if G_df[columns[experiment]].isna().sum() > 0:
            G_df[columns[experiment]].fillna('Control', inplace=True)
...
def update_column(r):
    return r['מפוצציםcracked'] + r['נפליםfallen'] + r['רקוביםrotten'] + r['רכיםsoft'] + r['גמישיםflexible'] + r[
        'מוצקיםfirm'] + r["חסריםmissing"] + r["חריגי צבעcolor defect"]
...
def extract_quality(df, experiment):
    ...
    Quality_df = pd.DataFrame({'SampleID': df[columns['מזהה דגימה']].astype(int),
                              ...
                              index=df.index)
    if experiment:
        df['ExperimentType'] = experiment
        if df[columns[experiment]].isna().sum() > 0:
            df[columns[experiment]].fillna('Control', inplace=True)

        df['ExperimentParameter'] = df[columns[experiment]]
        experiment_cols = {'ExperimentType': df['ExperimentType'].astype(str),
                           'ExperimentParameter': df['ExperimentParameter'].astype(str)}
...
#######################################################################################################################
# the main Integration function, all the calls begins here.

# first we call "extract_meta" which calls integration functions as writen above after the "extract_meta" function.
# there is the user interaction.
# second we iterate on all sorting type extract the relevent data no need for any thing.
# after we insert the data for the relevnt tables need to print messeges for the user only no interaction.
# i will mark where each step begins in the integration function.
...
def Integrate(file_path):
    ...
    experiment = ' '.join(experiment_user.split())
    columns = columns_mapper(df)
    # 1. calling "extract_meta" need user ineraction
    meta, df = extract_meta(df)
    sorting_types = df[columns['סוג מיון']].unique().tolist()
    all_indices_to_remove = []
    all_indices_to_keep = []
    q, c, y = None, None, None

    # 2. calling extract for each sorting type no need to do.
    ...
    # 3. calling insert for each sorting type including meta and for what type if there is experiment or not.
    # meta dosent change if there is experiment or not.
    if len(meta) != 0:
        Insert_Meta(meta)
        if experiment:
            if c is not None:
        ...
        print(
            'the data is already in the data base, or you gave the authirity to remove corrputed data which is all the file data.')

    return


Integrate("../temp_files/Experiment108Results.csv")